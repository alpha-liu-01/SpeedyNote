...
bool TouchGestureHandler::handleTouchEvent(QTouchEvent* event)
{
    ...
    if (event->type() == QEvent::TouchUpdate) {
        ...
        if (m_activeTouchPoints == 1 && !m_panActive && !activePoints.isEmpty()) {
            // BUG-A005 v7: If pinch is active, DON'T immediately transition to pan.
            // After hideEvent (tab switch), Android may send spurious Released events
            // that temporarily drop finger count to 1. If we immediately end the pinch,
            // the gesture becomes unreliable.
            // 
            // Instead, keep pinch "paused" until:
            // - Second finger reappears (resume pinch)
            // - TouchEnd with 0 fingers (end everything)
            // - Stale timeout removes the phantom finger (then transition naturally)
            if (m_pinchActive) {
            ...
            // Coming from 3+ fingers (not pinch), start pan with remaining finger
            const auto& point = *activePoints.first();
            m_lastPos = point.position();
            m_panActive = true;
