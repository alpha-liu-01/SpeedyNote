...
**File:** `source/MainWindow.cpp`, `source/ui/sidebars/PagePanel.h/cpp`
**Changes:**
...
### Code Review: Phase P (Post-Implementation)
...
|----|----------|-------------|--------|
| CR-P.1 | Medium | `TimelineDelegate::m_thumbnailCache` not invalidated when `NotebookLibrary::thumbnailUpdated` is emitted. Stale thumbnails would be shown after updates. | ✅ Fixed |
| CR-P.2 | Low | `NotebookLibrary::cleanupThumbnailCache()` had a `qDebug()` statement for production code. | ✅ Fixed |
| CR-P.3 | Trivial | `Launcher::hideWithAnimation()` had redundant `disconnect()` call - `Qt::SingleShotConnection` already auto-disconnects. | ✅ Fixed |
| CR-P.4 | Note | `NotebookLibrary` singleton is never explicitly deleted. This is standard for app-wide Qt singletons; OS cleans up at exit. | N/A |
| CR-P.5 | Note | `TimelineDelegate::m_thumbnailCache` is unbounded but tied to Launcher lifecycle. Acceptable since Launcher is typically one instance per app lifetime. | N/A |

**Fixes Applied:**

1. **CR-P.1: TimelineDelegate thumbnail cache invalidation**
   - Added `TimelineDelegate::invalidateThumbnail(bundlePath)` slot
   - Added `TimelineDelegate::clearThumbnailCache()` helper
   - Connected `NotebookLibrary::thumbnailUpdated` to `TimelineDelegate::invalidateThumbnail`
   - Added `#include "../../core/NotebookLibrary.h"` to both files

2. **CR-P.2: Remove qDebug from NotebookLibrary**
   - Removed debug print in `cleanupThumbnailCache()`

3. **CR-P.3: Simplify hideWithAnimation**
   - Removed redundant `disconnect()` call since `Qt::SingleShotConnection` is used

**Additional Bug Fix (Tab Close Warning):**
- Found that `DocumentViewport::documentModified` signal was not connected to mark `Document` as modified or update tab's modified flag
- Added connection in `MainWindow::connectViewportScrollSignals()`:
  - Calls `doc->markModified()` when content is modified
  - Calls `m_tabManager->markTabModified(tabIndex, true)` to show * in tab title
- This fix ensures the save prompt appears when closing tabs with unsaved changes
